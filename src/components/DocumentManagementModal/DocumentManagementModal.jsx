...
const DocumentManagementModal = ({ mode, document, onClose, onSuccess, onViewRequest }) => {
    ...
    const handlePreviewClick = (version) => {
        const isCompleted = document.status === 'completed' && document.signedFileUrl;
        const urlToView = isCompleted ? document.signedFileUrl : version.url;
        onViewRequest(urlToView);
    };
            ...
            <div className="bg-white dark:bg-slate-800 rounded-xl shadow-xl w-full max-w-2xl border border-slate-200/80 dark:border-slate-700/50 flex flex-col">
                ...
                <div className="p-6 max-h-[70vh] overflow-y-auto">
                    <form onSubmit={handleSubmit} className="space-y-4">
                        <div>
                            <label htmlFor="title" className="block mb-2 text-sm font-medium text-slate-700 dark:text-slate-300">Judul Dokumen</label>
                            <input
                                type="text"
                                id="title"
                                value={title}
                        ...
                        <div>
                            <label htmlFor="file-upload" className="block mb-2 text-sm font-medium text-slate-700 dark:text-slate-300">{mode === "create" ? "Pilih File" : "Unggah Versi Baru (Opsional)"}</label>
                            <input
                                type="file"
                                id="file-upload"
                                        ...
                                        return (
                                            <li key={version.id} className={`p-3 rounded-lg flex items-center justify-between transition-colors ${isActive ? "bg-blue-100 dark:bg-blue-900/50" : "bg-slate-50 dark:bg-slate-800"}`}>
                                                ...
                                                <div className="flex items-center space-x-1">
                                                    <button onClick={() => handlePreviewClick(version)} className="p-2 text-slate-600 hover:text-blue-600 dark:text-slate-300 dark:hover:text-blue-400 rounded-full hover:bg-slate-200 dark:hover:bg-slate-700" title={isCompleted ? "Lihat Hasil TTD" : "Lihat Versi Ini"}>
                                                        <FaEye />
                                                    </button>
                                                    <a href={isCompleted ? document.signedFileUrl : version.url} target="_blank" rel="noopener noreferrer" className="p-2 text-slate-600 hover:text-blue-600 dark:text-slate-300 dark:hover:text-blue-400 rounded-full hover:bg-slate-200 dark:hover:bg-slate-700" title={isCompleted ? "Unduh Hasil TTD" : "Unduh Versi Ini"}>
                                                        <FaDownload />
                                                    </a>
                                                    {!isActive && (
                                                        <>
                                                            <button onClick={() => handleUseVersion(version.id)} className="p-2 text-slate-600 hover:text-green-600 dark:text-slate-300 dark:hover:text-green-400 rounded-full hover:bg-slate-200 dark:hover:bg-slate-700" title="Gunakan Versi Ini">
                                                                <FaUndo />
                                                            </button>
                                                            <button onClick={() => handleDeleteVersion(version.id)} className="p-2 text-slate-600 hover:text-red-600 dark:text-slate-300 dark:hover:text-red-400 rounded-full hover:bg-slate-200 dark:hover:bg-slate-700" title="Hapus Versi Ini">
                                                                <FaTrash />
                                                            </button>
                                                        </>
                                                    )}
