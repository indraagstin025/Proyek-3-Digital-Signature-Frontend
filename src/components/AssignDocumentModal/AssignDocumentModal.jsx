...
import { useGetMyDocuments } from '../../hooks/useDocumentGroup'; 
import { useAssignDocumentToGroup } from '../../hooks/useGroups';
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl shadow-2xl w-full max-w-2xl flex flex-col max-h-[90vh] border border-slate-200 dark:border-slate-700">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
            ...
            <div>
                ...
                {isLoadingDocs ? (
                    <div className="flex justify-center py-8 text-slate-500"><ImSpinner9 className="animate-spin mr-2"/> Memuat dokumen...</div>
                ) : availableDocuments.length === 0 ? (
                    <div className="p-4 bg-slate-50 dark:bg-slate-900 rounded-lg text-center text-slate-500 border border-dashed border-slate-300 dark:border-slate-700">
                        Tidak ada dokumen draft yang tersedia.
                    </div>
                ) : (
                        ...
                        {availableDocuments.map(doc => (
                            ...
                            >
                                ...
                                <div className="flex-1 min-w-0">
                                    <p className={`font-medium truncate ${selectedDocumentId === doc.id ? 'text-blue-700 dark:text-blue-300' : 'text-slate-700 dark:text-slate-300'}`}>
                                        {doc.title}
                                    </p>
            ...
            {selectedDocumentId && (
                <div className="animate-in fade-in slide-in-from-bottom-4 duration-300">
                    ...
                    <h4 className="text-sm font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 mb-3">
                        2. Siapa yang wajib tanda tangan?
                    </h4>
                    ...
                    <p className="text-xs text-slate-400 mt-2 ml-1">
                        *Dokumen akan berstatus "Pending" sampai user terpilih melakukan tanda tangan.
                    </p>
                </div>
            )}
        </div>
        ...
        <div className="p-5 border-t border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50 rounded-b-xl flex justify-end gap-3">
          <button
            onClick={onClose}
            className="px-5 py-2.5 rounded-lg text-sm font-medium text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors"
          >
            Batal
          </button>
          <button
