...
import { FaCheck, FaRobot, FaSignature, FaCheckCircle, FaEye, FaCog, FaTrashAlt } from "react-icons/fa";

const DocumentCard = ({ 
  doc, 
  isSelected, 
  onToggle, 
  onNavigate, // Ini sekarang fungsi generik dari parent
  onManage, 
  onDelete, 
  getStatusClass, 
  getTypeColor, 
  ...
  return (
    <article
      onClick={() => !isCompleted && onToggle(doc.id)}
      className={`group relative flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 p-5 bg-white dark:bg-slate-800 rounded-2xl border transition-all duration-200 cursor-pointer
        ${isSelected 
      ...
      <div className="flex items-start gap-4 w-full sm:flex-1">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex flex-wrap items-center gap-2 mb-1.5">
            <h3 className="text-base font-bold text-slate-800 dark:text-white truncate max-w-[200px] sm:max-w-md" title={doc.title}>
              {doc.title}
            </h3>
      ...
      <div className="flex items-center gap-3 w-full sm:w-auto justify-end border-t sm:border-t-0 border-slate-100 dark:border-slate-700 pt-3 sm:pt-0 mt-2 sm:mt-0" onClick={(e) => e.stopPropagation()}>
        {!isActionDone ? (
          <button
            ...
            // Panggil onNavigate tanpa parameter path hardcode.
            // Biarkan 'handleSmartNavigation' di parent yang menentukan tujuannya.
            onClick={() => onNavigate()} 
        ...
        <div className="flex items-center gap-1 bg-slate-100 dark:bg-slate-700/50 p-1 rounded-lg">
          {/* View biasanya universal, tapi kalau mau smart nav juga bisa onNavigate('view') */}
