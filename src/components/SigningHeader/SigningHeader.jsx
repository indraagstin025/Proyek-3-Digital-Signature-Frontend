...
const SigningHeader = ({ theme, toggleTheme, onToggleSidebar, activeUsers = [], currentUser }) => {
  const allUsers = [
    {
    ...
    <header className="fixed top-0 left-0 w-full z-50 flex items-center justify-between h-16 px-4 shadow-sm backdrop-blur-sm sm:px-6 lg:px-8 bg-white/80 border-b border-slate-200/80 dark:bg-slate-900/80 dark:border-white/10">
      ...
      <div className="flex items-center gap-3 sm:gap-4">
        ...
        <div className="flex -space-x-2 overflow-hidden mr-2 items-center">
              ...
              >
                {finalImageUrl ? (
                  <>
                    ...
                    <div className={`hidden items-center justify-center w-full h-full rounded-full ${user.color || getColor(user.userName)}`}>{getInitials(user.userName)}</div>
                  </>
                ) : (
                  /* Tampilan Inisial (Jika URL null) */
                  <div className={`flex items-center justify-center w-full h-full rounded-full ${user.color || getColor(user.userName)}`}>{getInitials(user.userName)}</div>
                )}
              </div>
            );
          })}
          ...
          {remainingCount > 0 && <div className="flex items-center justify-center w-8 h-8 text-[10px] font-bold text-white bg-slate-500 rounded-full ring-2 ring-white dark:ring-slate-800 shadow-sm">+{remainingCount}</div>}
        </div>
