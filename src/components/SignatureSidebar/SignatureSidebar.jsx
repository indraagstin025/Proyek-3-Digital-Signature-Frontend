  ...
  return (
    <aside
      className={`
        ...
        bg-slate-50 dark:bg-slate-900/90 backdrop-blur-sm
    ...
    >
        ...
        <div>
          <h2 className="text-xl font-bold text-slate-800 dark:text-white flex items-center gap-2">
            ...
            {isSignedSuccess ? (
                ...
                <span className="bg-yellow-100 text-yellow-700 text-[10px] px-2 py-0.5 rounded-full border border-yellow-200">View Only</span>
            )}
          </h2>
      ...
      <div className="flex-1 p-6 space-y-6 overflow-y-auto min-h-0 custom-scrollbar">
        ...
        <div className={`bg-white dark:bg-slate-800 p-4 rounded-xl shadow-md border border-slate-200/80 dark:border-slate-700 transition-opacity ${readOnly || isSignedSuccess ? 'opacity-60 grayscale-[0.5]' : ''}`}>
          ...
          {savedSignatureUrl ? (
            <div className="group relative flex justify-center">
              <div 
                className={`draggable-signature w-32 aspect-square p-2 border-2 border-slate-300 dark:border-slate-600 rounded-xl bg-white dark:bg-slate-700 flex justify-center items-center shadow-sm transition-all
                  ${(readOnly || isSignedSuccess) ? 'cursor-not-allowed border-dashed' : 'cursor-grab hover:shadow-md hover:border-blue-400 active:cursor-grabbing'}
                `}
              >
                <img src={savedSignatureUrl} alt="Tanda Tangan" className="w-full h-full object-contain pointer-events-none select-none" />
              </div>
              
              {/* Tombol Ubah disembunyikan jika ReadOnly atau Sudah Sukses */}
              {!readOnly && !isSignedSuccess && (
                <button onClick={onOpenSignatureModal} className="absolute -top-2 -right-2 text-xs font-semibold text-white bg-blue-600 hover:bg-blue-700 py-1.5 px-3 rounded-full shadow-lg opacity-0 group-hover:opacity-100 transition-all transform hover:scale-105">
                  Ubah
                </button>
              )}
            </div>
        ...
        {/* --- 2. AI TOOLS (Hanya Legal Assistant) --- */}
        <div className={`bg-gradient-to-br from-indigo-50 to-purple-50 dark:from-slate-800 dark:to-slate-800 p-4 rounded-xl shadow-sm border border-indigo-100 dark:border-slate-700 ${readOnly ? 'opacity-60' : ''}`}>
           <div className="flex items-center gap-2 mb-3">
             <div className="p-1.5 bg-indigo-100 text-indigo-600 rounded-lg dark:bg-slate-700 dark:text-indigo-400"><FaRobot size={14} /></div>
             <p className="text-sm font-bold text-slate-700 dark:text-white">Legal Assistant</p>
           </div>
           <button 
             onClick={onAnalyze} 
             disabled={isLoading} // Fitur Analyze tetap boleh meski sudah sign (hanya baca)
