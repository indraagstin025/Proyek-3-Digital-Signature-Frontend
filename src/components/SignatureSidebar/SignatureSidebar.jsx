...
// [TAMBAHAN] Tambahkan prop 'hasPlacedSignature'
const SignatureSidebar = ({ 
  ...
  return (
    <aside
      className={`
        select-none w-full max-w-xs lg:w-80 lg:max-w-md
        ...
        fixed top-16 right-0 z-40 h-[calc(100dvh-4rem)]
      ...
      <div className="p-6 border-t border-slate-200/80 dark:border-slate-700/50 mt-auto bg-slate-50/50 dark:bg-slate-800/30 shrink-0 z-20 pb-24 lg:pb-6">
        {!isSignedSuccess && (
          <div className={`flex items-center justify-between mb-4 ${readOnly ? "opacity-50 pointer-events-none" : ""}`}>
            ...
            <button id="qr-toggle" onClick={() => setIncludeQrCode(!includeQrCode)} disabled={readOnly} className={`relative inline-flex items-center h-6 rounded-full w-11 transition-colors duration-200 focus:outline-none ${includeQrCode ? "bg-blue-600" : "bg-slate-300 dark:bg-slate-600"}`}>
        ...
        <button
          ...
          // 1. Jika Loading -> Disable
          // 2. Jika belum sukses DAN tidak punya gambar ttd -> Disable
          // 3. Jika belum sukses DAN readOnly -> Disable
          // 4. Jika belum sukses DAN belum taruh ttd di kanvas -> Disable (Konsisten dgn Floating Button)
          disabled={
            isLoading || 
            (!savedSignatureUrl && !isSignedSuccess) || 
            (readOnly && !isSignedSuccess) ||
