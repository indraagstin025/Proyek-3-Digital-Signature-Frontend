...
import { FaPenNib, FaTimes, FaRobot, FaLock, FaEye, FaSpinner, FaHandPointer } from "react-icons/fa";
...
const SignatureSidebar = ({
  ...
  // --- LOGIKA INTERACT.JS (KODE LAMA/STABIL) ---
  useEffect(() => {
      ...
      listeners: {
        start(event) {
          ...
          Object.assign(clone.style, {
            ...
            // 🔥 Pastikan Clone juga background Putih agar terlihat saat di-drag
  ...
  return (
    ...
    >
      ...
      <div className="py-6 px-6 border-b border-slate-200/80 dark:border-slate-700/50 flex justify-between items-center shrink-0">
        <div>
          <h2 className="text-xl font-bold text-slate-800 dark:text-white flex items-center gap-2">
            Panel Kontrol
            {isSignedSuccess ? (
              <span className="bg-green-100 text-green-700 text-[10px] px-2 py-0.5 rounded-full border border-green-200 animate-pulse">Selesai</span>
            ) : (
              readOnly && <span className="bg-yellow-100 text-yellow-700 text-[10px] px-2 py-0.5 rounded-full border border-yellow-200">View Only</span>
            )}
          </h2>
        ...
        <button onClick={onClose} className="p-2 text-slate-500 hover:text-slate-800 dark:hover:text-white portrait:inline-flex landscape:hidden">
          <FaTimes size={20} />
        </button>
      </div>
      ...
      <div className="flex-1 p-6 space-y-6 overflow-y-auto min-h-0 custom-scrollbar">
        {/* --- AREA TANDA TANGAN (TARGET TOUR 1) --- */}
        <div id="tour-signature-area" className={`bg-white dark:bg-slate-800 p-4 rounded-xl shadow-md border border-slate-200/80 dark:border-slate-700 transition-opacity ${readOnly || isSignedSuccess ? "opacity-60 grayscale-[0.5]" : ""}`}>
          ...
          {savedSignatureUrl ? (
            <div className="group relative flex flex-col items-center">
              ...
              <div className="relative">
                {/* 🔥 PERUBAHAN DI SINI:
                    - 'bg-white' dipaksa (tanpa dark:bg-...) agar tanda tangan selalu terlihat jelas.
                    - Border disesuaikan agar tetap terlihat rapi di kedua mode.
                */}
                <div
                  className={`draggable-signature w-32 aspect-square p-2 border-2 rounded-xl bg-white flex justify-center items-center shadow-sm transition-all 
                  ${readOnly || isSignedSuccess ? "cursor-not-allowed border-dashed border-slate-300" : "cursor-grab hover:shadow-md hover:border-blue-400 active:cursor-grabbing border-slate-300"}`}
                >
              ...
              {/* 🔥 INSTRUKSI VISUAL (HINT) */}
              {!readOnly && !isSignedSuccess && (
                <div className={`mt-3 flex items-center gap-2 text-slate-400 ${showDragHint ? "animate-bounce text-blue-600 font-bold" : "animate-pulse"}`}>
                  <FaHandPointer size={12} className="transform rotate-180" />
                  <p className="text-xs">Tekan & Tarik ke dokumen</p>
                </div>
              )}
            </div>
          ) : (
        ...
        {/* --- AREA AI ASSISTANT (TARGET TOUR 2) --- */}
        <div id="tour-ai-assistant">
          ...
          <div className={`bg-gradient-to-br from-indigo-50 to-purple-50 dark:from-slate-800 dark:to-slate-800 p-4 rounded-xl shadow-sm border border-indigo-100 dark:border-slate-700 mt-4 ${readOnly ? "opacity-60" : ""}`}>
      ...
      <div className="p-6 border-t border-slate-200/80 dark:border-slate-700/50 mt-auto bg-slate-50/50 dark:bg-slate-800/30 shrink-0 z-20 pb-24 lg:pb-6">
        {!isSignedSuccess && (
          <div className={`flex items-center justify-between mb-4 ${readOnly ? "opacity-50 pointer-events-none" : ""}`}>
            ...
            <button
              ...
              disabled={readOnly}
              className={`relative inline-flex items-center h-6 rounded-full w-11 transition-colors duration-200 focus:outline-none ${includeQrCode ? "bg-blue-600" : "bg-slate-300 dark:bg-slate-600"}`}
            >
        ...
        >
          {isLoading ? (
            <>
              <FaSpinner className="animate-spin mr-3" /> Memproses...
            </>
          ) : isSignedSuccess ? (
            <>
              <FaEye className="mr-2" /> Lihat Hasil Tanda Tangan
            </>
          ) : readOnly ? (
            "Mode Lihat Saja"
          ) : (
