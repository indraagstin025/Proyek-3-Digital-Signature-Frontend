...
import { 
  ...
  FaChartPie, FaFolderOpen, FaArrowUp, FaEllipsisH
} from "react-icons/fa";
...
const StatCard = ({ icon, label, value, colorTheme }) => {
  // Color presets dengan gradasi halus
  const themes = {
    blue: {
  ...
  return (
    <div className="relative overflow-hidden bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-lg shadow-slate-200/50 dark:shadow-none border border-slate-100 dark:border-slate-700 transition-transform duration-300 hover:-translate-y-1">
      <div className="flex justify-between items-start z-10 relative">
        <div>
          ...
          <div className="flex items-baseline gap-2">
            ...
            {/* Indikator dummy untuk estetika */}
...
const ActionCard = ({ doc, navigate }) => {
  ...
  if (isGroupTask) {
    ...
    // Routing Logic
    if (doc.groupId) {
      buttonAction = () => navigate(`/dashboard/workspaces`); // Sesuai request Anda
    } else {
  ...
  return (
    <div className={`group relative bg-white dark:bg-slate-800 rounded-xl p-5 border border-slate-100 dark:border-slate-700 shadow-sm hover:shadow-md transition-all duration-300 ${cardBorder}`}>
      <div className="flex flex-col sm:flex-row gap-4 justify-between items-start sm:items-center">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-2">
            <span className={`text-[10px] px-2.5 py-1 rounded-md font-bold uppercase tracking-wide ${badgeStyle}`}>
              {badgeLabel}
            </span>
...
const DashboardOverview = ({ theme }) => {
  ...
  // Chart Config
  const radius = 60; // Sedikit lebih kecil agar elegan
  ...
  return (
    <div className="h-full w-full overflow-y-auto custom-scrollbar bg-slate-50/50 dark:bg-[#0f172a]">
      <div id="tab-overview" className="mx-auto max-w-7xl px-4 pt-8 pb-24 sm:px-6 lg:px-8 space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <section>
              <div className="flex items-center justify-between mb-4">
                ...
                {data?.actions?.length > 0 && (
                  <span className="bg-rose-100 text-rose-600 text-xs font-bold px-3 py-1 rounded-full dark:bg-rose-900/30 dark:text-rose-400 shadow-sm">
                    {data.actions.length} Pending
                  </span>
                )}
              </div>
                    ...
                    <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                        ...
                        filteredActivities.map((activity, index) => {
                            ...
                            >
                                ...
                                <td className="px-4 py-4 text-center">
                                    <span className={`px-3 py-1 inline-flex text-[10px] font-bold rounded-full uppercase tracking-wide ${statusBadge}`}>
