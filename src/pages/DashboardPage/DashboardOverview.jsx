...
const StatCard = ({ icon, label, value, colorTheme }) => {
  ...
  return (
    <div
      className={`relative overflow-hidden p-6 rounded-2xl transition-transform duration-300 hover:-translate-y-1 
      bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 
      shadow-lg shadow-slate-200/50 dark:shadow-slate-900/50 ${theme.darkGlow}`}
    >
      {/* GLOW CORNERS */}
      <div className={`absolute -top-12 -right-12 w-32 h-32 rounded-full blur-3xl opacity-20 dark:opacity-30 ${theme.cornerColor} pointer-events-none`}></div>
      <div className={`absolute -bottom-12 -left-12 w-24 h-24 rounded-full blur-2xl opacity-10 dark:opacity-20 ${theme.cornerColor} pointer-events-none`}></div>
      ...
      <div className="flex justify-between items-start z-10 relative">
        ...
        <div className={`p-3.5 rounded-2xl ${theme.bg} text-white shadow-lg ${theme.shadow}`}>
...
const ActionCard = ({ doc, navigate }) => {
  ...
  // Tentukan warna sudut berdasarkan tipe task
  let cornerColor = "bg-blue-500"; 
  ...
  if (isGroupTask) {
    ...
    cornerColor = "bg-purple-500"; // Ungu untuk grup
    buttonAction = () => navigate(`/dashboard/documents/group`);
    ...
    cornerColor = "bg-rose-500"; // Merah untuk personal
    buttonAction = () => navigate(`/dashboard/documents`);
  } else {
    buttonLabel = "Lihat";
    badgeLabel = doc.status || "File";
    buttonAction = () => navigate(`/dashboard/documents/personal`);
  }

  return (
    <div
      className={`group relative overflow-hidden rounded-xl p-5 transition-all duration-300 ${cardBorder} 
      bg-white dark:bg-slate-800 border-y border-r border-slate-100 dark:border-slate-700 
      shadow-sm hover:shadow-md dark:shadow-slate-900/20 dark:hover:shadow-blue-900/10`}
    >
      {/* GLOW CORNERS (Lebih kecil untuk kartu list) */}
      <div className={`absolute -top-8 -right-8 w-24 h-24 rounded-full blur-2xl opacity-10 dark:opacity-20 ${cornerColor} pointer-events-none transition-opacity group-hover:opacity-20 dark:group-hover:opacity-30`}></div>

      {/* Konten (Relative z-10 agar di atas glow) */}
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-slate-500 mt-1 flex items-center gap-1">
            ...
            <span className="truncate">
              Dari: <span className="font-semibold text-slate-700 dark:text-slate-300">{doc.ownerName || "Saya"}</span>
            </span>
          </p>
        </div>
        <button
          onClick={buttonAction}
          className={`shrink-0 flex items-center justify-center gap-2 py-2.5 px-5 rounded-lg text-sm font-semibold transition-all active:scale-95 w-full sm:w-auto mt-2 sm:mt-0 shadow-sm ${
            isUrgent
              ? "text-white bg-slate-900 hover:bg-black dark:bg-white dark:text-slate-900 dark:hover:bg-slate-200"
              : "text-slate-600 bg-white border border-slate-200 hover:bg-slate-50 dark:bg-slate-800 dark:border-slate-600 dark:text-slate-300 dark:hover:bg-slate-700"
          }`}
        >
...
const DashboardOverview = ({ theme }) => {
  ...
  return (
    <div className="relative h-full w-full overflow-y-auto custom-scrollbar bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100 dark:from-slate-900 dark:via-slate-800 dark:to-slate-900 transition-colors duration-300">
      <OnboardingTour tourKey="dashboard_intro" steps={DASHBOARD_STEPS} onOpenSidebar={() => setSidebarOpen && setSidebarOpen(true)} />
      ...
      <div className="fixed inset-0 z-0 pointer-events-none overflow-hidden">
        ...
        <div className="hidden lg:block absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-80 h-80 bg-purple-500/10 dark:bg-purple-500/5 rounded-full blur-3xl animate-pulse delay-1000"></div>
      </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-8 items-start">
          ...
          <div className="xl:col-span-2 space-y-8">
            ...
            <section id="dashboard-actions">
              ...
              <div className="space-y-4">
                {data?.actions?.length > 0 ? (
                  ...
                  <div className="relative overflow-hidden p-10 text-center rounded-2xl border border-dashed border-slate-300 dark:border-slate-700 bg-white/60 dark:bg-slate-800/60 backdrop-blur-xl">
                    {/* Glow Empty State */}
                    <div className="absolute -top-10 -right-10 w-32 h-32 rounded-full blur-3xl opacity-10 bg-emerald-500 pointer-events-none"></div>
            ...
            <section id="dashboard-activity">
              {/* Added: Relative & Overflow Hidden for Glow */}
              <div className="relative overflow-hidden rounded-2xl shadow-sm bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700">
                {/* Glow Corners for Activity */}
                <div className="absolute -top-20 -right-20 w-48 h-48 rounded-full blur-3xl opacity-10 dark:opacity-20 bg-indigo-500 pointer-events-none"></div>
                
                <div className="relative z-10 p-5 border-b border-slate-100 dark:border-slate-700 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  <h3 className="text-lg font-bold text-slate-800 dark:text-white flex items-center gap-2">
                    ...
                    {[
                      ...
                      <button
                        ...
                        className={`px-3 py-1.5 text-xs font-semibold rounded-md transition-all duration-200 ${
                          activityFilter === tab.id ? "bg-white dark:bg-slate-600 text-indigo-600 dark:text-white shadow-sm" : "text-slate-500 hover:text-slate-700 dark:text-slate-400"
                        }`}
                      >
                        {tab.label}
                      </button>
                    ...
                    <tbody className="divide-y divide-slate-100 dark:divide-slate-700/50">
                      {filteredActivities.length > 0 ? (
                        filteredActivities.map((activity, index) => {
                          ...
                          return (
                            <tr key={`${activity.id}-${index}`} onClick={() => navigate(`/dashboard/documents/personal`)} className="group hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors cursor-pointer">
                              <td className="px-6 py-4">
                                <div className="flex items-center gap-3">
                                  ...
                                  <div>
                                    <p className="font-semibold text-sm text-slate-700 dark:text-slate-200 group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors truncate max-w-[180px] sm:max-w-xs">
                                      {activity.title}
                                    </p>
                                  </div>
                                </div>
          ...
          <div className="xl:col-span-1">
            <div className="sticky top-6">
              ...
              {/* SUMMARY CARD */}
              {/* Added: Relative & Overflow Hidden for Glow */}
              <div id="dashboard-summary" className="relative overflow-hidden rounded-2xl shadow-sm p-6 bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700">
                {/* Glow Corners for Summary (Emerald/Green Theme) */}
                <div className="absolute -top-12 -right-12 w-32 h-32 rounded-full blur-3xl opacity-10 dark:opacity-20 bg-emerald-500 pointer-events-none"></div>
                <div className="absolute -bottom-12 -left-12 w-24 h-24 rounded-full blur-2xl opacity-10 dark:opacity-20 bg-emerald-500 pointer-events-none"></div>
                ...
                <div className="relative z-10 flex flex-col items-center">
                  <div className="relative w-48 h-48 mb-8 group cursor-default">
                    ...
                    <div className="absolute inset-0 flex flex-col items-center justify-center z-10">
                      ...
                      <span className="text-xs font-bold text-slate-400 uppercase tracking-widest mt-1">Selesai</span>
                    </div>
                  </div>
