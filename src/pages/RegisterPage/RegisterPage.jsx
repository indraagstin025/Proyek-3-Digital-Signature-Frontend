    ...
    if (isRegistered) {
        return (
            <div className="flex items-center justify-center min-h-screen px-4 py-12">
                <div className={`w-full max-w-sm text-center bg-white dark:bg-gray-900/80 p-8 rounded-2xl shadow-lg border border-slate-200 dark:border-white/10`}>
                    <h1 className="text-2xl font-bold text-slate-900 dark:text-white mb-4">Pendaftaran Berhasil!</h1>
                    <p className="text-slate-600 dark:text-gray-400 mb-6">
                        Silakan cek inbox email Anda untuk memverifikasi akun.
                    </p>
                    <Link to="/login" className="w-full inline-block bg-gradient-to-r from-blue-600 to-cyan-500 text-white font-semibold py-3 px-4 rounded-lg hover:opacity-90 transition-opacity">
        ...
        <div className="flex items-center justify-center min-h-screen px-4 py-12">
            <div className={`w-full max-w-sm bg-white dark:bg-gray-900/80 dark:backdrop-blur-sm p-8 rounded-2xl shadow-lg border border-slate-200 dark:border-white/10 transition-all duration-700 ease-out ${isCardVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"}`}>
                <div className="text-center mb-8">
                    ...
                    <h1 className="text-2xl font-bold text-slate-900 dark:text-white mt-4">Buat Akun Baru</h1>
                    <p className="text-slate-600 dark:text-gray-400">Bergabunglah bersama kami sekarang.</p>
                </div>
                
                <form onSubmit={handleRegister} className="space-y-4">
                    <div>
                        <label htmlFor="name" className="block text-sm font-medium text-slate-700 dark:text-gray-300 mb-1">Nama Lengkap</label>
                    ...
                    <div>
                        <label htmlFor="email" className="block text-sm font-medium text-slate-700 dark:text-gray-300 mb-1">Alamat Email</label>
                        <div className="relative">
                            <span className="absolute inset-y-0 left-0 flex items-center pl-3">
                    ...
                    <div>
                        <label htmlFor="password" className="block text-sm font-medium text-slate-700 dark:text-gray-300 mb-1">Password</label>
                        <div className="relative">
                            <span className="absolute inset-y-0 left-0 flex items-center pl-3">
                    ...
                    <div className="pt-2">
                        <button type="submit" disabled={loading} className="w-full flex items-center justify-center bg-gradient-to-r from-blue-600 to-cyan-500 text-white font-semibold py-3 px-4 rounded-lg hover:opacity-90 transition-opacity transform hover:scale-[1.02] duration-300 disabled:opacity-50 disabled:scale-100">
                            {loading ? (<><ImSpinner9 className="animate-spin h-5 w-5 mr-3" /> Memuat...</>) : ("Buat Akun")}
                        </button>
                    </div>
                </form>
                ...
                <div className="mt-8 text-center">
                    <p className="text-sm text-slate-600 dark:text-gray-400">
                        Sudah punya akun?
                        <Link to="/login" className="font-semibold text-blue-500 dark:text-blue-400 hover:underline ml-1">
                            Login di sini
                        </Link>
                    </p>
                </div>
