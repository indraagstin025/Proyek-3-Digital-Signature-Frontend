  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-lg w-full max-w-lg p-6 relative">
        ...
        <h2 className="text-xl font-semibold text-gray-900 dark:text-gray-100 mb-4">Ubah Foto Profil</h2>
          ...
          {preview ? (
            <div className="flex flex-col items-center space-y-4">
              ...
              <div className="flex space-x-2">
                <button onClick={handleUpload} disabled={isUploading} className="px-4 py-2 bg-blue-600 text-white rounded-lg shadow hover:bg-blue-700 disabled:opacity-60 flex items-center gap-2">
                  {isUploading ? (
                    <>
            ...
            <label htmlFor="upload-input" className="cursor-pointer flex flex-col items-center justify-center space-y-2">
        ...
        {profilePictures.length === 0 ? (
          <p className="text-sm text-gray-500 dark:text-gray-400 text-center">Belum ada riwayat foto.</p>
        ) : (
          <div className="grid grid-cols-3 gap-4 max-h-64 overflow-y-auto">
            {profilePictures.map((pic) => (
              <div key={pic.id} className="relative group border rounded-lg overflow-hidden">
                <img src={pic.url} alt="Foto Profil Lama" className="w-full h-28 object-cover" />
                <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition flex items-center justify-center gap-2">
                  <button onClick={() => handleUseOldPicture(pic.id)} className="bg-green-500 hover:bg-green-600 text-white p-2 rounded-full">
                    <FaCheck />
                  </button>
                  <button onClick={() => handleDelete(pic.id)} disabled={isDeleting === pic.id} className="bg-red-500 hover:bg-red-600 text-white p-2 rounded-full">
