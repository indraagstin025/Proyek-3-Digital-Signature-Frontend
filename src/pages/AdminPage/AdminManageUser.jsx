                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    {!isEditMode && (
                        <div>
                            ...
                            <input type="password" name="password" placeholder='Minimal 8 karakter' value={formData.password || ''} onChange={handleChange} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm dark:bg-slate-700 dark:border-slate-600" required />
                        </div>
                    )}
                     <div className="flex items-center">
                        ...
                        <label htmlFor="isSuperAdmin" className="ml-3 block text-sm text-slate-900 dark:text-gray-200">Jadikan Super Admin</label>
                    </div>

                    <div className="flex justify-end gap-4 pt-4">
                        <button type="button" onClick={onClose} className="px-4 py-2 rounded-lg text-sm font-semibold bg-gray-200 hover:bg-gray-300 dark:bg-gray-600 dark:hover:bg-gray-500">Batal</button>
                        <button type="submit" className="px-4 py-2 rounded-lg text-sm font-semibold text-white bg-blue-600 hover:bg-blue-700">Simpan</button>
                    </div>
                </form>
            </div>
        </div>
...
const AdminManageUser = ({ theme }) => {
    ...
    const handleSaveUser = async (formData) => {
        const promise = modalState.user
            ? adminService.updateUser(modalState.user.id, formData)
        ...
        <div className="mx-auto max-w-screen-xl space-y-6">
                 ...
                 <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className={`bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-xl w-full max-w-sm ${theme}`}>
                        ...
                        <div className="flex justify-end gap-4 mt-6">
                            <button onClick={() => setDeleteConfirm({isOpen: false, userId: null})} className="px-4 py-2 rounded-lg text-sm font-semibold bg-gray-200 hover:bg-gray-300 dark:bg-gray-600 dark:hover:bg-gray-500">Batal</button>
                            <button onClick={() => handleDeleteUser(deleteConfirm.userId)} className="px-4 py-2 rounded-lg text-sm font-semibold text-white bg-red-600 hover:bg-red-700">Ya, Hapus</button>
                        </div>
                    </div>
                </div>
            )}
            ...
            <div className="flex justify-between items-center">
                ...
                <button 
                    ...
                    className="flex items-center gap-2 px-4 py-2 rounded-lg font-semibold bg-blue-600 text-white hover:bg-blue-700 transition-colors shadow-lg shadow-blue-500/30"
                >
                ...
                <div className="overflow-x-auto">
                    <table className="min-w-full divide-y divide-slate-200 dark:divide-slate-700">
                        ...
                        <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
                            {loading ? (
                                <tr><td colSpan="4" className="text-center py-8 text-slate-500">Memuat data pengguna...</td></tr>
                            ) : (
                                users.map(user => (
                                    <tr key={user.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50">
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            <span className={`px-2.5 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${user.isSuperAdmin ? 'bg-green-100 text-green-800 dark:bg-green-900/50 dark:text-green-300' : 'bg-blue-100 text-blue-800 dark:bg-blue-900/50 dark:text-blue-300'}`}>{user.isSuperAdmin ? 'Admin' : 'User'}</span>
                                        </td>
                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-500 dark:text-slate-400">
                                            {new Date(user.createdAt).toLocaleDateString('id-ID', { year: 'numeric', month: 'long', day: 'numeric' })}
                                        </td>
                                        <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <button onClick={() => setModalState({ isOpen: true, user: user })} className="text-indigo-600 hover:text-indigo-900 dark:text-indigo-400 dark:hover:text-indigo-300 p-2 rounded-full hover:bg-slate-100 dark:hover:bg-slate-700"><HiPencil className="h-5 w-5"/></button>
                                            <button onClick={() => setDeleteConfirm({ isOpen: true, userId: user.id })} className="text-red-600 hover:text-red-900 dark:text-red-400 dark:hover:text-red-300 p-2 rounded-full hover:bg-slate-100 dark:hover:bg-slate-700 ml-2"><HiTrash className="h-5 w-5"/></button>
                                        </td>
                                    </tr>
                                ))
                            )}
